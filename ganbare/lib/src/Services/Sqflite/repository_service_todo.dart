...
class RepositoryServiceTodo {
  static Future<List<Todo>> getAllTodos() async {
    ...
    List<String> todosNotificar = List();
    ...
    for (final node in data) {
      ...
      if (!todo.isDeleted) {
        todosNotificar.add(todo.name);
        print("get all todos " + todo.name);
      }
    }

    // Agregar el guardado en Shared preferences
    SharedPreferences preferences = await SharedPreferences.getInstance();

    preferences.setStringList("todosEnMemoria", todosNotificar);
    //preferences.reload();
    return todos;
  }
